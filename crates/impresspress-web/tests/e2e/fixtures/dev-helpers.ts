...
/**
 * Getting a browser to the point where `/b/dev` exists.
 *
 * Every dev-sandbox spec starts the same way — boot the service worker, sign
 * in as the seeded admin — and none of it is what any of them is testing, so
 * it lives here rather than in whichever spec was written first.
 *
 * These helpers are for the SANDBOX bundle (`examples/dev-sandbox/build.sh`,
 * served on `TEST_PORT`), not for the native server the rest of the suite
 * uses. That is why they log in through the form instead of reusing
 * `fixtures/auth.ts`'s saved `storageState`: `global-setup.ts` posts to a
 * server that is not running in this job, and the sandbox's credentials are
