...
pub const RELEASE_ASSET_KEYS_VAR: &str = "IMPRESSPRESS_RELEASE_ASSET_KEYS_JSON";
/// Cloudflare currently limits each Worker variable to 5 KB. Keep margin for
/// serialization/config overhead and fail before upload if the exact key set
/// cannot be represented safely in one request-current variable.
pub const MAX_RELEASE_ASSET_KEYS_VAR_BYTES: usize = 4 * 1024;
...
fn generate_named(
    ...
    if let Some(release) = release {
        let logical_keys_json = release.logical_keys_json()?;
        if logical_keys_json.len() > MAX_RELEASE_ASSET_KEYS_VAR_BYTES {
            bail!(
                "release asset key set is {} bytes; maximum is {} bytes for {}. \
                 Reduce/shorten release-managed logical keys or split the release set",
                logical_keys_json.len(),
                MAX_RELEASE_ASSET_KEYS_VAR_BYTES,
        ...
        vars.insert(
            RELEASE_ASSET_KEYS_VAR.into(),
            toml::Value::String(logical_keys_json),
        );
        vars.insert(
