...
mod tests {
    ...
    use super::{
        artifact_sha256, parse_labeled_line, r2_upload_release_with_client, resolve_secret,
        ...
        CONCURRENT_SMOKE_MAX_IN_FLIGHT, CONCURRENT_SMOKE_TOTAL_REQUESTS,
    };
    ...
    async fn concurrent_smoke_matches_mixed_p32_acceptance_workload() {
        ...
        tokio::time::timeout(
            ...
            smoke_authenticated_concurrency(&preview_url, token, &paths),
        )
        .await
    ...
    async fn concurrent_smoke_rejects_empty_path_slice() {
        let error = smoke_authenticated_concurrency("http://127.0.0.1:1", "token", &[])
            .await
            .unwrap_err();
    ...
    fn release_upload_writes_only_immutable_objects_and_preserves_logical_keys() {
        ...
        assert_eq!(report.metadata_objects_uploaded, 2);
