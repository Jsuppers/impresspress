...
use impresspress::cli::helpers::cloudflare::{
    assets::{ReleaseAssetEntry, ReleaseManifest},
    ...
    wrangler::{
        ...
        MAX_RELEASE_ASSET_KEYS_VAR_BYTES, PREPARED_APPLICATION_BUILD_SHA256_VAR,
        ...
        RELEASE_ASSET_KEYS_VAR, RELEASE_ASSET_MANIFEST_SHA256_VAR, RELEASE_ASSET_MANIFEST_VAR,
...
fn generate_upload_binds_release_identity_and_exact_key_set_after_overrides() {
    ...
    let keys: Vec<String> =
        serde_json::from_str(vars[RELEASE_ASSET_KEYS_VAR].as_str().unwrap()).unwrap();
    assert_eq!(keys, vec!["site/app.js", "site/media/hero.webp"]);
}

#[test]
fn generate_upload_rejects_release_key_set_larger_than_worker_var_budget() {
    let tmp = tempdir().unwrap();
    let out = tmp.path().join("target/impresspress-cloudflare");
    fs::create_dir_all(&out).unwrap();
    let long_key = "x".repeat(MAX_RELEASE_ASSET_KEYS_VAR_BYTES);
    let release = ReleaseManifest {
        schema_version: 1,
        asset_set_sha256: "a".repeat(64),
        immutable_prefix: format!(".impresspress/releases/v1/{}", "a".repeat(64)),
        files: vec![ReleaseAssetEntry {
            logical_key: long_key,
            size: 0,
            sha256: "b".repeat(64),
    ...
    let err =
        generate_upload_with_release(&sample_cfg(), tmp.path(), &out, Some(&release)).unwrap_err();

    assert!(err.to_string().contains(RELEASE_ASSET_KEYS_VAR));
