...
mod tests {
    ...
    /// the grouping in `fetch_block_variables` drops every row without one:
    /// "Shared (`WAFER_RUN_SHARED__*`) and ad hoc keys have no block and
    /// belong to no block's snapshot."
    ///
    /// Nothing else picks them up. `structural_runtime_config` builds the
    /// Cloudflare `ConfigService` map from worker secrets, builder-time worker
    /// vars and the block-settings JSON only — "after dropping the D1 env_vars
    /// pre-load" — so a shared row reaches neither the async service nor the
    /// synchronous snapshot. On Workers the site's primary colour, app name
    /// and logo are therefore whatever the code defaults to no matter what an
    /// admin saved, and unlike native no restart fixes it.
    ///
    /// The assertion is the observable requirement — an admin-saved shared
    /// value is what a block reads — expressed through `load_for_block`,
    /// the only D1-backed config read this target has today. The config-store
    /// decision may instead surface these rows through a variables-backed
    /// CONFIG_GET sharing this source's snapshot; if it does, re-point this
    /// test at that surface rather than deleting it. What must not change is
    ...
    async fn a_shared_variables_row_is_readable_on_cloudflare() {
        ...
        let config = src
            .load_for_block(
                "impresspress/admin",
                &[var("WAFER_RUN_SHARED__PRIMARY_COLOR")],
            )
            .await
            .expect("load config for the admin block");

        assert_eq!(
            config.get("WAFER_RUN_SHARED__PRIMARY_COLOR"),
            Some("#ff0000"),
            "a shared variables row an admin saved must reach config reads on \
             Cloudflare; dropping it is why branding changes never appear on \
             Workers whatever the admin saves"
        );
    }
}
