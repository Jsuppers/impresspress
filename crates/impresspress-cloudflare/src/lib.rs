...
fn make_d1_database_service_concrete(
    ...
    // (`ctx.wait_until`, below) and for the pre-Init window, and each gets a
    // service whose `strict_schema` is still `false` and whose `SchemaCache` is
    // brand new. `create_many` then calls `ensure_data_columns`, which
    // introspects, and the cache can never be warm because the service does not
    // outlive the request.
    //
