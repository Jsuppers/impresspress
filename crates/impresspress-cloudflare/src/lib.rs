...
///
/// Returns `Ok(None)` only when no release contract is configured or the
/// logical key is not an exact inventory member. A partial, malformed, or
/// digest-mismatched contract fails closed so direct R2 fast paths cannot
/// silently downgrade to mutable logical objects.
pub fn release_asset_object_key(
    env: &worker::Env,
    logical_key: &str,
) -> worker::Result<Option<String>> {
    request_services::ReleaseAssetIdentity::from_env(env)
        .map(|identity| identity.and_then(|identity| identity.physical_object_key(logical_key)))
...
struct VerifiedIdentityCache<T> {
    entry: Option<(String, std::rc::Rc<T>)>,
}

impl<T> Default for VerifiedIdentityCache<T> {
    fn default() -> Self {
        Self { entry: None }
    }
}

impl<T> VerifiedIdentityCache<T> {
    fn get_or_try_insert_with<E>(
        &mut self,
        key: String,
        load: impl FnOnce() -> Result<T, E>,
    ) -> Result<std::rc::Rc<T>, E> {
        if let Some((cached_key, value)) = &self.entry {
            if cached_key == &key {
                return Ok(value.clone());
            }
        }
        let value = std::rc::Rc::new(load()?);
...
thread_local! {
    ...
    static PREPARED_PLAN_CACHE: std::cell::RefCell<VerifiedIdentityCache<impresspress_core::PreparedRuntimePlan>> =
        std::cell::RefCell::new(VerifiedIdentityCache::default());
}
...
pub(crate) fn runtime_environment_identity(
    ...
    let release_manifest = env
        ...
        .map(|value| value.to_string())
        .unwrap_or_default();
    let release_keys = env
        .var(request_services::RELEASE_ASSET_KEYS_JSON_VAR)
        .map(|value| value.to_string())
        .unwrap_or_default();
    ...
    let components = [
        ...
        ("release-keys", release_keys.as_str()),
...
async fn prepared_verify_endpoint(
    ...
    let result = async {
        ...
        // Parse the exact Worker KEYS_JSON routing contract used by ordinary
        // request reads. Manifest verification alone is insufficient: a
        // self-consistent manifest cannot detect a different routing
        // inventory bound into the Worker version.
        ...
        } = &identity.release_assets
        {
            ...
            let routing = release_routing
                .as_deref()
                .ok_or("prepared release has no Worker routing inventory")?;
            if routing.logical_keys_sorted() != logical_keys {
                return Err("Worker routing inventory differs from release manifest".into());
            }
            ...
            if let Some(entry) = manifest.files.first() {
                let immutable_key = routing
                    .physical_object_key(&entry.logical_key)
                    .ok_or("representative asset is absent from Worker routing inventory")?;
...
mod request_config_tests {
    ...
    fn verified_identity_cache_loads_once_and_invalidates_on_identity_change() {
        let mut cache = VerifiedIdentityCache::default();
