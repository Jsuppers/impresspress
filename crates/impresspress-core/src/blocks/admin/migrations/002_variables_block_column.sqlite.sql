-- Add a `block` column to impresspress__admin__variables for indexed per-block
-- lookup by D1ConfigSource (lazy-init redesign).
--
-- Format: SCREAMING_SNAKE block prefix derived from the `key` column's first
-- two `__`-delimited segments. e.g. key `WAFER_RUN__AUTH__JWT_SECRET` ->
-- block `WAFER_RUN__AUTH`. Keys without two segments (legacy or shared
-- WAFER_RUN_SHARED__*) leave `block` NULL.

ALTER TABLE impresspress__admin__variables ADD COLUMN block TEXT;

-- Backfill existing rows. SQLite-only: find the second `__` in `key` and
-- take the substring up to (but not including) that position. Keys with
-- fewer than two `__` get NULL.
UPDATE impresspress__admin__variables
SET block = CASE
    WHEN instr(substr(key, instr(key, '__') + 2), '__') > 0
        THEN substr(key, 1, instr(key, '__') + 1 + instr(substr(key, instr(key, '__') + 2), '__') - 1)
    ELSE NULL
END
WHERE block IS NULL;

CREATE INDEX IF NOT EXISTS impresspress__admin__variables_block_idx
    ON impresspress__admin__variables (block);
