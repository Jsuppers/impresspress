...
// `mod.rs`'s `BlockInfo::collections(repo::products::TABLE, ...)` is an
// advisory table listing for admin/WRAP discovery, not a query — nothing to
// filter. `tests/` fixtures seed rows (including soft-deleted ones) and
// assert on raw rows directly against the table; that's setup/assertion for
// the tests guarding the door, not a production read that must respect the
// filter.
const TABLE_IDENT_ALLOWED: &[&str] = &[
    "repo/products.rs", // the door itself: `TABLE`'s own definition and its internal uses
    "mod.rs",
    "tests/",
];
