...
- Names other plans rely on: `blocks::dev::templates::{wafer_guest.rs, hello, table, reference.md}`, `blocks::dev::scaffold`, `WAFER_GUEST_VERSION`, `window.ImpresspressCompiler`, `/__impresspress_dev/compiler/manifest.json`, `examples/dev-sandbox/compiler/{PIN.json, build-compiler.sh, dist/}`, e2e `fixtures/{dev-helpers.ts, fake-compiler-worker.js}`, tools `dev_create_block`, `dev_read_reference`, `dev_compile_block`.
