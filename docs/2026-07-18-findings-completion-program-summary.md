...
- **Optional future producer follow-ups (non-blocking, documented):** fold the cloud-metadata-host denylist into `wafer-net-security` so native fetch + registry downloads share it; expose the storage cursor codec so backends can share one encoder. Neither was needed for this program (all consumer changes reused the current pin).
