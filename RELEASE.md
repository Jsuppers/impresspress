...
per user and role. Before, two logins of the bootstrap-admin address at the same
...
issued to them while they had it.

**The repair deletes rows.** Migration `004_user_roles_unique` deletes
duplicate grant rows before creating the index, keeping the **earliest** row for
each user and role (by `created_at`, `id` as the tie-break). Every row it
deletes repeats a grant the kept row still makes, so nobody gains or loses a
role — what goes is the extra row a revoke could miss.
